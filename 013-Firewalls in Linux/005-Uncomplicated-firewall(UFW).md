...
- ⚙️ Easy rule management commands such as enabling/disabling the firewall, viewing rules, deleting specific rules, and resetting all rules.
...
- 🖥️ Well-suited for host-based firewalls and is the default firewall management tool on many Debian-based Linux distributions such as Ubuntu.

Overall, UFW bridges the complexity of iptables configuration with an accessible, straightforward approach to securing Linux systems through firewall rules.

---

## Installing UFW 🛠️(Debain)
...
- 🖥️ **Allow TCP port 24 from specific IP:**
  ```bash
  ufw allow from 192.168.1.52 to any port 24 proto tcp
  ```

- 🖥️ **Allow TCP port 30 to specific IP:**
  ```bash
...
- Delete all rules and reset UFW:**
  ```bash
  ufw reset
  ```